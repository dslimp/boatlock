...
void test_run_command_parses_csv_payload() {
  const RuntimeSimCommand command = parseRuntimeSimCommand("SIM_RUN:S1,1");

  TEST_ASSERT_EQUAL((int)RuntimeSimCommandType::RUN, (int)command.type);
  TEST_ASSERT_TRUE(command.valid);
  TEST_ASSERT_EQUAL_STRING("S1", command.scenarioId.c_str());
  TEST_ASSERT_EQUAL(1, command.speedup);
}

void test_run_command_defaults_to_fast_mode() {
  const RuntimeSimCommand command = parseRuntimeSimCommand("SIM_RUN:S2");

  TEST_ASSERT_EQUAL((int)RuntimeSimCommandType::RUN, (int)command.type);
  TEST_ASSERT_TRUE(command.valid);
  TEST_ASSERT_EQUAL_STRING("S2", command.scenarioId.c_str());
...
void test_run_command_rejects_unsupported_payload_forms() {
  const RuntimeSimCommand objectPayload =
      parseRuntimeSimCommand("SIM_RUN {id=S2,speedup=1}");
  const RuntimeSimCommand spacedPayload = parseRuntimeSimCommand("SIM_RUN S2,1");
