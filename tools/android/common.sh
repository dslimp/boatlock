...
BOATLOCK_ANDROID_SIGNING_ENV="${BOATLOCK_ANDROID_SIGNING_ENV:-${HOME:-}/.boatlock/android-release-signing.env}"
BOATLOCK_APP_CHECK_MODES=(basic reconnect manual status sim sim_suite anchor compass gps ota)

if [[ -n "${BOATLOCK_ANDROID_SIGNING_ENV}" && -r "${BOATLOCK_ANDROID_SIGNING_ENV}" ]]; then
  # shellcheck source=/dev/null
  source "${BOATLOCK_ANDROID_SIGNING_ENV}"
  export BOATLOCK_ANDROID_KEYSTORE_PATH="${BOATLOCK_ANDROID_KEYSTORE_PATH:-}"
  export BOATLOCK_ANDROID_KEYSTORE_PASSWORD="${BOATLOCK_ANDROID_KEYSTORE_PASSWORD:-}"
  export BOATLOCK_ANDROID_KEY_ALIAS="${BOATLOCK_ANDROID_KEY_ALIAS:-}"
  export BOATLOCK_ANDROID_KEY_PASSWORD="${BOATLOCK_ANDROID_KEY_PASSWORD:-}"
  export BOATLOCK_ANDROID_REQUIRE_RELEASE_SIGNING="${BOATLOCK_ANDROID_REQUIRE_RELEASE_SIGNING:-1}"
fi
