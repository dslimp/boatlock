...
from anchor_sim import SimConfig, default_scenarios, simulate_scenario
...
def check_thresholds(scenario_name: str, metrics: Dict[str, float]) -> List[str]:
    ...
    if min_dead is not None and metrics["time_in_deadband_pct"] < min_dead:
        errs.append(
            f"time_in_deadband_pct={metrics['time_in_deadband_pct']:.2f} < {min_dead:.2f}"
        )
    if max_p95 is not None and metrics["p95_error_m"] > max_p95:
        errs.append(f"p95_error_m={metrics['p95_error_m']:.2f} > {max_p95:.2f}")
    if max_max is not None and metrics["max_error_m"] > max_max:
...
def main() -> int:
    ...
    print("name | deadband% | p95(m) | max(m) | sat% | dirChanges/min | status")
    print("-" * 90)
