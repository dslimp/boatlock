...
- Work in batches of fifteen modules by default.
...
- Run `nh02` hardware acceptance and Android BLE smokes after every fifteenth module in the batch.
- Do not wait for the fifteenth module when the change directly touches hardware drivers, pinout, deploy/debug wrappers, actuator safety, BLE reconnect/install behavior, or another path where local tests cannot bound the risk.
